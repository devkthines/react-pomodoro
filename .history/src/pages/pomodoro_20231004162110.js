...
export default function ClockFace() {
  ...
  const [speakerColor, setSpeakerColor] = use
  ...
  return (
    <div className={themeClass} id='watchBand'>
      ...
      <div id='watch'>
      <BsSoundwave className='speakers'/>  <div id='clockFace'><h1 id='time-left'>{Converter(timer)}</h1></div>   <BsSoundwave className='speakers'/>
      </div>

      <div id='controls'>
       <div className='timeslide'>

        <div> <h6 className={whatIsIt === 'BREAK' ? 'no-bold' : 'bold'}>Break: &nbsp; {Converter(breakT)}</h6>
       ...
       <div><Button type="primary" shape="circle" icon={<GrPowerReset />} onClick={onResetClick} />
        <Switch
      ...
      <Faqs />
      <footer><h3>&copy;2023 Korey's Coding Solutions</h3></footer>
    </div>
  )
}
