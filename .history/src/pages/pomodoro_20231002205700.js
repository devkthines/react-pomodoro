...
export default function ClockFace() {
  ...
  const [theme,setTheme] = useState(<GrMoon></GrMoon>)
  const starter = useRef(null);

  let song = new Audio(sound)

  const darkLight = (e) =>{
    theme === <GrMoon></GrMoon> ? setTheme(<GiSun></GiSun>) : setTheme(<GrMoon></GrMoon>)
  }
  ...
  return (
    <div id='watchBand'>
      ...
      <div id='buttons'>
        ...
        <button className='controls' onClick={()=>{darkLight()}}>{theme}</button>
